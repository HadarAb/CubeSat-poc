/* Implements the payload-side I2C VTable register protocol and HAL callbacks. */
...
uint8_t our_node_id = PAYLOAD_NODE_ID;
...
void PrepareValueResponse(void)
{
    ...
    transmit_value.crc16 =
    		Protocol_Crc16(reinterpret_cast<const uint8_t*>(&transmit_value), VT_VALUE_CRC_SIZE);
}
											...
											uint16_t address_match_code)
{
    ...
    if (transfer_direction == I2C_DIRECTION_TRANSMIT)
    {
        ...
        receive_state = ReceiveState::Register;
        (void)HAL_I2C_Slave_Seq_Receive_IT(hi2c, &received_register, 1u, I2C_FIRST_FRAME);
    ...
    {
        case REG_WHOAMI:
            our_node_id = PAYLOAD_NODE_ID;
            // send back slave ID
            StartTransmit(hi2c, &our_node_id, sizeof(transmit_node_id));
            break;

        case REG_VT_COUNT:
            transmit_count = VTable_Count();
            StartTransmit(hi2c, reinterpret_cast<uint8_t*>(&transmit_count),
            											sizeof(transmit_count));
            break;

        case REG_VT_VALUE:
            PrepareValueResponse();
            StartTransmit(hi2c, reinterpret_cast<uint8_t*>(&transmit_value),
                          VT_VALUE_WIRE_SIZE);
            break;

        case REG_VT_ENTRY:
            PrepareEntryResponse();
            StartTransmit(hi2c, reinterpret_cast<uint8_t*>(&transmit_entry),
                          VT_ENTRY_WIRE_SIZE);
            break;
...
extern "C" void HAL_I2C_SlaveRxCpltCallback(I2C_HandleTypeDef* hi2c)
{
    ...
    if (receive_state == ReceiveState::Register)
    {
        ...
        if (selected_register == REG_VT_SELECT)
        {
            ...
            (void)HAL_I2C_Slave_Seq_Receive_IT(hi2c, reinterpret_cast<uint8_t*>(selected_key),
            										VT_NAME_LEN, I2C_LAST_FRAME);
        }
        ...
        else if (selected_register == REG_VT_AT)
        {
            ...
            (void)HAL_I2C_Slave_Seq_Receive_IT(
                hi2c, received_index, sizeof(received_index), I2C_LAST_FRAME);
        }
    ...
    {
        selected_index = (uint16_t)received_index[0]
            | (uint16_t)((uint16_t)received_index[1] << 8u);
