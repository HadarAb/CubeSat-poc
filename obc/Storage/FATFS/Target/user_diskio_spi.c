...
#define FCLK_SLOW() { __HAL_SPI_DISABLE(&SD_SPI_HANDLE); \
                      MODIFY_REG(SD_SPI_HANDLE.Instance->CR1, SPI_CR1_BR, SPI_BAUDRATEPRESCALER_256); }

#define FCLK_FAST() { __HAL_SPI_DISABLE(&SD_SPI_HANDLE); \
...
//	for (n = 10; n; n--) xchg_spi(0xFF);	/* Send 80 dummy clocks */
	FCLK_SLOW();
	CS_HIGH();
	HAL_Delay(10);                          /* let the card turn on */
	for (n = 10; n; n--) xchg_spi(0xFF);    /* 80 clocks */
