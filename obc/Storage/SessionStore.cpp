...
void copy_slot_to_metadata(const SessionSlot_t& slot, SessionMetadata_t* metadata)
{
...
bool SessionStore_Load(const char* dir_path, SessionMetadata_t* metadata, bool* valid)
{
    ...
    *valid = false;
    ...
    for (uint32_t slot_index = 0u; slot_index < 2u; ++slot_index) {
        ...
        slot_valid[slot_index] = (bytes_read == sizeof(SessionSlot_t)) && session_slot_is_valid(slots[slot_index]);
    }
    ...
    if (slot_valid[0] && slot_valid[1]) {
        if (generation_is_newer(slots[1].generation, slots[0].generation)) {
            copy_slot_to_metadata(slots[1], metadata);
        } else {
            copy_slot_to_metadata(slots[0], metadata);
        }
        *valid = true;
