											...
											uint8_t* crc_valid,uint32_t* calculated_crc)
{
    ...
    *crc_valid = 0u;
    ...
    HAL_StatusTypeDef status = ReadRegister(slave_address,REG_DATA,
    ...
    const uint32_t crc = Protocol_Crc32(
        reinterpret_cast<const uint8_t*>(payload_data),PAYLOAD_DATA_CRC_SIZE);
