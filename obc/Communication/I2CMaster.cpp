...
namespace
{
constexpr uint32_t i2c_timeout_ms = 20u;
 ...
 */
HAL_StatusTypeDef read_register(uint16_t slave_address, uint8_t register_address,
                               uint8_t* output, uint16_t output_size)
{
    return HAL_I2C_Mem_Read(master_i2c, slave_address, register_address,
                            I2C_MEMADD_SIZE_8BIT, output, output_size, i2c_timeout_ms);
}
}
...
HAL_StatusTypeDef i2c_master_read_who_am_i(uint16_t slave_address, uint8_t* node_id)
{
    ...
    return read_register(slave_address, REG_WHOAMI, node_id, 1u);
}
                                     ...
                                     VtType_t expected_type, VtValueWire_t* value)
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(master_i2c,slave_address,
    								select_request, sizeof(select_request),i2c_timeout_ms);
    ...
    status = read_register(slave_address,REG_VT_VALUE,reinterpret_cast<uint8_t*>(value),
    								VT_VALUE_WIRE_SIZE);
    if (status != HAL_OK) {
        return I2C_KEY_READ_BUS_ERROR;
    }
